  ...
  topics:
    ...
    - id: reliable-bot-delivery
      ...
      code_anchors:
        ...
        - path: bot-service/internal/kafka/consumer.go
          symbol: "Consumer.Run — if err := c.core.handle(ctx, msg.Value); err != nil { ... continue } / затем CommitMessages"
          concept: "offset коммитится ТОЛЬКО если handle не вернул ошибку — то же сообщение будет прочитано заново на следующей итерации цикла"
        - path: bot-service/internal/kafka/consumer_test.go
          symbol: "TestFailedSendIsRetriedNotDeduplicated / flakySender"
          concept: "тест, который сначала заставляет отправку упасть, а потом проверяет: то же событие доставляется повторно и ровно один раз успешно, а не теряется и не дублируется"
      quiz_seeds:
      ...
      decisions:
        - choice: "и `remember(eventID)`, и коммит Kafka-offset происходят строго ПОСЛЕ успешного `SendMessage` — если отправка падает, `Consumer.Run` не коммитит offset и повторяет то же сообщение через 3 секунды, блокируя обработку следующих сообщений партиции до тех пор, пока это не удастся"
          alternatives: "коммитить offset сразу после чтения (до отправки) и просто логировать ошибку отправки — тогда очередь не блокируется, но сообщение, включая коды подтверждения, можно молча потерять"
...
запоминается. То же самое зеркально на уровне Kafka-транспорта:
...
`continue` возвращает выполнение в начало того же цикла `for ctx.Err() ==
nil`, а `CommitMessages` стоит ПОСЛЕ успешной обработки — offset не
продвигается, пока `handle` не вернёт `nil`. Практическое следствие видно
прямо из структуры цикла: `FetchMessage` внутри того же `for` не читает
следующее сообщение партиции, пока текущее не обработано без ошибки. Если у
одного гостя доставка сломана навсегда (например, он заблокировал бота),
его сообщение будет повторяться каждые 3 секунды бесконечно и не пропустит
вперёд себя ничьи другие уведомления — включая OTP-коды других гостей — в
том же топике `notifications.outbound`. Это ровно то, что называется
head-of-line blocking, и спека признаёт эту цену явно: «Принятая цена:
...
Тест воспроизводит именно эту гарантию — не «сообщение дошло», а «сообщение
не потерялось и не задвоилось при сбое и повторе»:
...
> смотри `consumerCore.send` (`remember` после успеха) и `Consumer.Run`
> (условный `CommitMessages`). Открой
> `bot-service/internal/kafka/consumer_test.go` — смотри `flakySender` и
> `TestFailedSendIsRetriedNotDeduplicated`.
