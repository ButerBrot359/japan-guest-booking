...
диапазон хоть с одним из уже существующих» при каждой вставке. Первая строка миграции — `CREATE EXTENSION IF NOT EXISTS btree_gist;`. Стоит понимать
...
необходимым не является. Частичность (`WHERE (status IN ('PENDING_OTP', 'CONFIRMED'))`) означает, что
