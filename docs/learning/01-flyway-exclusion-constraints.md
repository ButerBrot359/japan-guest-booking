...
диапазон хоть с одним из уже существующих» при каждой вставке. Индекс подключается
через расширение `btree_gist` (первая строка миграции: `CREATE EXTENSION IF NOT EXISTS
btree_gist;`) — оно добавляет GiST-поддержку для обычных типов вроде `bigint`, что
здесь, впрочем, не используется впрямую (наш exclude — только по `daterange`), но
расширение обязательно для того, чтобы GiST вообще можно было применить в этом
контексте. Частичность (`WHERE (status IN ('PENDING_OTP', 'CONFIRMED'))`) означает, что
...
Комментарий в самой миграции это explicitly проговаривает: «Полуинтервал
