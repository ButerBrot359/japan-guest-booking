...
- Инфраструктурное: тестовые значения `app.jwt.secret` / `app.admin.*`
  берутся из основного application.yml (dev-заглушки; отдельный тестовый
  yml не заводился — уточнено по итогам финального ревью);
  TRUNCATE-список AbstractIntegrationTest не меняется (новых таблиц нет).
