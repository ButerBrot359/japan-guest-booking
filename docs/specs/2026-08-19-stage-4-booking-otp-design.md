...
- **Подтверждение** `POST /api/bookings/{id}/confirm {code}`: `verify` →
  ...
  - `RESCHEDULE`: атомарный UPDATE дат из `payload` челленджа; EXCLUDE-
    конфликт (даты заняли за 5 минут) → 409 `DATES_TAKEN`; исключение
    откатывает транзакцию confirm целиком, челлендж остаётся PENDING —
    гость может повторить confirm или запросить новый перенос; успех →
